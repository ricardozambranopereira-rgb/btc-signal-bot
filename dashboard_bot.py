...
# Parametros iguales al Pine Script
ZL_LENGTH = 70
BAND_MULT = 1.2

# Timeframes del panel MTF
MTF_TIMEFRAMES = {
    "5m": "5m",
    "15m": "15m",
    "1h": "1h",
...
# Evita repetir la misma direccion/evento
...
def calc_zero_lag(df):
    ...
    # Flechas pequenas:
    # ta.crossover(close, zlema) and trend == 1 and trend[1] == 1
...
def send_telegram(msg):
    if TELEGRAM_BOT_TOKEN == "PEGA_AQUI_TU_TOKEN_REAL":
    ...
    while True:
        try:
            ...
            print(
                ...
                data_actual["price"],
                "| Evento:",
                data_actual["entry_type"]
            )

            # Telegram solo por evento real Zero Lag
            current_event = data_actual["raw_signal"]

            if current_event in ["BUY", "SELL"]:
                event_key = f"{current_event}_{data_actual['entry_type']}"
...
body{
  ...
  color:var(--txt);
  background:
    linear-gradient(180deg,rgba(145,78,112,.92),rgba(48,43,78,.88) 32%,rgba(8,20,35,1) 100%),
    radial-gradient(circle at 20% 8%,rgba(56,189,248,.25),transparent 28%),
    radial-gradient(circle at 85% 8%,rgba(168,85,247,.22),transparent 30%);
  padding:30px;
}
...
.logo{
  ...
  background:conic-gradient(var(--green),var(--blue),#8b5cf6,#f59e0b,var(--green));
  box-shadow:0 0 28px rgba(56,189,248,.65);
}

h1{margin:0;font-size:29px;font-weight:900;letter-spacing:-1px}
...
.card h2,.card h3{margin:0 0 8px;font-weight:850;letter-spacing:-.4px}
.sub{color:var(--muted);font-size:13px}

.signalbox{text-align:center;min-height:500px;display:flex;flex-direction:column;justify-content:center}
...
.BUY,.BULLISH{color:var(--green);text-shadow:0 0 35px rgba(0,255,187,.75)}
.SELL,.BEARISH{color:var(--red);text-shadow:0 0 35px rgba(255,17,0,.75)}
.WAIT,.NEUTRAL{color:var(--yellow);text-shadow:0 0 35px rgba(250,204,21,.55)}

.gauge{height:250px;position:relative}
.gauge canvas{position:absolute;inset:0}
.gcenter{position:absolute;left:0;right:0;bottom:35px;text-align:center}
.gcenter b{font-size:56px;font-weight:950}
.gcenter small{display:block;color:var(--muted)}

.linebox{height:330px}

.mtf{margin-top:14px;display:grid;gap:8px}
...
.mtfrow span{padding:10px 12px;background:rgba(255,255,255,.05)}
.mtfrow b{padding:10px 12px;text-align:center}

.mtfrow .Bullish{background:rgba(0,255,187,.25);color:var(--green)}
.mtfrow .Bearish{background:rgba(255,17,0,.25);color:var(--red)}
.mtfrow .WAIT{background:rgba(250,204,21,.18);color:var(--yellow)}
...
.stat b{display:block;font-size:23px;margin-top:7px}
...
@media(max-width:1100px){
  body{padding:14px}
  .top,.grid,.lower,.stats{grid-template-columns:1fr;display:grid}
  .signal{font-size:50px}
}
</style>
</head>
...
<div class="shell">
  <div class="top">
    <div class="brand">
      <div class="logo"></div>
      <div>
        <h1>BTC Zero Lag Signals</h1>
        <div class="sub">Replica Python del Pine Script AlgoAlpha · Binance · {{d.interval}}</div>
      </div>
    </div>
    <div class="status">{{d.source}} · {{d.updated}}</div>
  </div>

  <div class="grid">
    <div class="card">
      <h2>MTF Trend Table</h2>
      <div class="sub">Mismo concepto de la tabla del indicador TradingView.</div>

      <div class="mtf">
        {% for tf, val in d.mtf.items() %}
        <div class="mtfrow">
          <span>{{tf}}</span>
          <b class="{{val}}">{{val}}</b>
        </div>
      ...
      <div class="note">
        Evento actual: <b>{{d.entry_type}}</b><br>
        Trend base: <b class="{{d.trend}}">{{d.trend}}</b>
      </div>
    </div>

    <div class="card signalbox">
      <div class="sub">Señal actual Zero Lag</div>
      <div class="signal {{d.signal}}">{{d.signal}}</div>

      <div class="gauge">
        <canvas id="gaugeChart"></canvas>
        <div class="gcenter">
          <b>{{d.score}}%</b>
          <small>Confluencia MTF</small>
        </div>
      </div>

      <div class="stats">
        <div class="stat"><small>Precio</small><b>{{d.price}}</b></div>
        <div class="stat"><small>ZLEMA</small><b>{{d.zlema}}</b></div>
        <div class="stat"><small>Volatilidad</small><b>{{d.volatility}}</b></div>
      </div>
    </div>

    <div class="card">
      <h2>Zero Lag Bands</h2>
      <div class="sub">Close + ZLEMA + bandas de volatilidad</div>
      <div class="linebox"><canvas id="lineChart"></canvas></div>
    </div>
  </div>
  ...
  <div class="lower">
    <div class="card">
      <h3>Trade Plan</h3>
      <div class="big {{d.signal}}">{{d.entry}}</div>
      <div class="sub">Entrada estimada</div>
      <div class="note">La entrada se genera por evento real Zero Lag, no por score.</div>
    </div>

    <div class="card">
      <h3>Bandas</h3>
      <div class="row"><span>Upper</span><b>{{d.upper}}</b></div>
      <div class="row"><span>ZLEMA</span><b>{{d.zlema}}</b></div>
      <div class="row"><span>Lower</span><b>{{d.lower}}</b></div>
    </div>

    <div class="card">
      <h3>Risk / Targets</h3>
      <div class="row"><span>SL</span><b>{{d.sl}}</b></div>
      <div class="row"><span>TP1</span><b>{{d.tp1}}</b></div>
      <div class="row"><span>TP2</span><b>{{d.tp2}}</b></div>
      <div class="row"><span>TP3</span><b>{{d.tp3}}</b></div>
    </div>
  </div>
</div>
...
const zlema = {{ d.zlema_series | tojson }};
const upper = {{ d.upper_series | tojson }};
const lower = {{ d.lower_series | tojson }};
const score = {{ d.score }};
...
new Chart(document.getElementById("lineChart"), {
  ...
  data:{
    ...
    datasets:[
      {label:"Close", data:prices, borderColor:"#38bdf8", backgroundColor:"rgba(56,189,248,.12)", fill:false, tension:.35, pointRadius:0, borderWidth:2},
      {label:"ZLEMA", data:zlema, borderColor:"#ffffff", fill:false, tension:.35, pointRadius:0, borderWidth:2},
      {label:"Upper", data:upper, borderColor:"#ff1100", fill:false, tension:.35, pointRadius:0, borderWidth:1},
      {label:"Lower", data:lower, borderColor:"#00ffbb", fill:false, tension:.35, pointRadius:0, borderWidth:1}
    ]
  },
  options:{
    responsive:true,
...
new Chart(document.getElementById("gaugeChart"), {
  ...
  data:{
    datasets:[{
      ...
      backgroundColor:[
        score >= 70 ? "#00ffbb" : score >= 50 ? "#facc15" : "#ff1100",
        "rgba(255,255,255,.08)"
      ],
      borderWidth:0,
      circumference:220,
